...
\begin{enumerate}
    \item We propose a conceptual meta-design framework based on five levels of data abstraction, enabling dataset designers to adapt complexity to user needs.
    \item We provide a formal definition of dataset complexity and demonstrate mathematically how transitions between abstraction levels reduce complexity.
...
\begin{figure}[h!]  % h = here, ! = force it
    \centering
    \includegraphics[width=0.4\linewidth]{figures/0.png}
    \caption{Table view of Level 0 data}
    \label{fig:level0-datum}
\end{figure}

\noindent At the lowest abstraction level, Level 0, data has no complexity as no interpretation is required for its comprehension. This type of data is commonly referred to as ``raw data.'' For instance, in our example, Michael weighs 45~kg, which is a fact.
...
\begin{figure}[h!]  % h = here, ! = force it
    ...
    \caption{Table view of Level 3 linkable datasets with multiple entity and attribute levels}
    \label{fig:level3-linkable}
\end{figure}
...
\subsubsection{Level 4 to Level 3}
...
\subsubsection{Level 3 to Level 2}
For the upper bound:
\begin{equation}
...
\subsubsection{Level 2 to Level 1}
\begin{equation}
...
\subsubsection{Level 1 to Level 0}
\begin{equation}
...
We implemented the framework as \texttt{intuitiveness}, an open-source Python package that operationalizes the descent-ascent methodology through three core capabilities: AI-assisted data modelling, semantic domain matching, and interactive navigation.
...
Educational programs and training initiatives can leverage the framework to scaffold learning through three key mechanisms. First, the framework enables \textbf{progressive skill development}---the five abstraction levels align with increasing data literacy competencies, allowing beginners to start at L0 (understanding single facts) and progressively advance to L3 (working with linked, multi-level structures). Second, it facilitates \textbf{flow-based learning experiences}---drawing on Csikszentmihalyi's flow theory, the framework positions data users to experience optimal challenge-skill balance at each level, reducing frustration and increasing engagement. Third, it supports \textbf{adaptive interfaces}---following recommendations from the visualization literacy literature \citep{alhamadi2022dataqualitymismatched, amyrotos2021adaptivevisualizationsfor}, our Streamlit interface implements progressive disclosure, minimal default visualizations, and contextual guidance that adapts to user actions.
...
% TODO: Develop with concrete future directions:
% - User studies to validate intuitiveness gains
% - Integration with existing open data platforms
% - Automated complexity assessment tools
% - Extension to streaming/real-time data

\textit{[This section requires development with specific future directions.]}
...
Not applicable. This research does not involve human subjects, animal subjects, or use of protected data. The case study involved analysis of metadata from an international logistics operator with appropriate data usage agreements.
...
The Python package \texttt{intuitiveness} implementing the meta-design framework is available as open-source software at https://github.com/[REPOSITORY-URL-NEEDED]. The case study data from the international logistics operator cannot be shared due to commercial confidentiality agreements. Sample datasets demonstrating the framework's capabilities are included in the package repository.
...
\newblock \doi{10.1145/3546155.3546708}
 ...
 \bibitem[Amyrotos, 2021]{amyrotos2021adaptivevisualizationsfor}
 Amyrotos, C. (2021).
 \newblock Adaptive Visualizations for Enhanced Data Understanding and Interpretation.
...
\newblock \textit{MIS Quarterly}, 28(1), 75--105.
 ...
 \newblock VizRec: Recommending Personalized Visualizations.
...
\newblock Springer.
 ...
 \newblock Toward a Taxonomy for Adaptive Data Visualization in Analytics Applications.
...
\newblock \textit{Information Polity}, 22(1), 1--24.
 ...
 \bibitem[Steichen et al., 2013]{steichen2013useradaptiveinformationvisualization}
 Steichen, B., Carenini, G., \& Conati, C. (2013).
 \newblock User-adaptive information visualization: using eye gaze data to infer visualization tasks and user cognitive abilities.
